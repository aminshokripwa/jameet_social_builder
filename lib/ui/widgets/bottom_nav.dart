...
class BottomNavigationJameet extends StatelessWidget {

  final int index;
  final bool isReel;

  const BottomNavigationJameet({Key? key, required this.index, this.isReel = false}) : super(key: key);
...
class _ItemButtom extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        child: ( isIcon ) 
          ? Icon(icon, color: ( i == index ) ? ColorsJameet.primary : isReel ?Colors.white : Colors.black87 , size: 28)
